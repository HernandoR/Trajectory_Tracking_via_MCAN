...
def saveRoadMap(shp_path, savePath, center, n):
    ...
    # create an ellipse
    radius_lat = (
        geopy.distance.distance(kilometers=5).destination(point=center, bearing=0)[0]
        - center[0]
    )
    radius_lon = (
        geopy.distance.distance(kilometers=5).destination(point=center, bearing=90)[1]
        - center[1]
    )
    ...
    folium.PolyLine(list([c[::-1] for c in ellipse.exterior.coords])).add_to(
        ellipse_map
    )

    # roads data
    berlin = roads_berlin.loc[
        roads_berlin["geometry"].apply(lambda g: ellipse.contains(g))
    ].copy()
...
config = yaml.safe_load(open("Datasets/profile.yml", "r"))
config = config["Cities"]
center = config[city]["center"]
shp_path = config[city]["shp_path"]
savePath = config[city]["roadmap_filepath"]
min_MaxSpeed = config[city]["min_MaxSpeed"]
